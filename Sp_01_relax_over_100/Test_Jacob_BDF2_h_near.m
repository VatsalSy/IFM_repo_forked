...
spine_lengths = .9*spine_lengths_ini;
spine_lengths(1) = spine_lengths_ini(1);
...
u_bar = ones(n_v_near,1)+.5*randomness*rand(n_v_near,1);
w_bar = ones(n_v_near,1)+.5*randomness*rand(n_v_near,1); % -1*(1-exp(-Nodes_rz(1:n_v_near,2)));%
u_far = ones(n_v-n_v_pre_start_far,1)+.5*randomness*rand(n_v-n_v_pre_start_far,1);
w_far = ones(n_v-n_v_pre_start_far,1)+.5*randomness*rand(n_v-n_v_pre_start_far,1);% -1*(1-exp(-Nodes_rz(n_v_start_far:end,2)));%

u_minus1 = ones(n_v,1)+.5*randomness*rand(n_v,1);
u_minus2 = u_minus1/2;
w_minus1 = ones(n_v,1)+.5*randomness*rand(n_v,1);% -1*(1-exp(-Nodes_rz(:,2)));%
w_minus2 = w_minus1/2;
u_s = ones(n_spines,1)+.5*randomness*rand(n_spines,1);
w_s = ones(n_spines,1)+.5*randomness*rand(n_spines,1);
A = 1;
...
p = ones(n_p,1)+.5*randomness*rand(n_p,1);

us1 = ones(n_spines,1)+.5*randomness*rand(n_spines,1);
ws1 = ones(n_spines,1)+.5*randomness*rand(n_spines,1);
rhos1 = Dg*ones(n_spines,1)+.5*randomness*rand(n_spines,1);
save('rhos1_0.mat','rhos1')
rhos1_minus1 = Dg*ones(n_spines,1)+.5*randomness*rand(n_spines,1);
rhos1_minus2 = Dg*ones(n_spines,1)/2;
sigma1 = ones(n_spines,1)+.5*randomness*rand(n_spines,1);
save('sigma1_0.mat','sigma1')
pg = ones(n_spines,1)+.5*randomness*rand(n_spines,1);
 
lambda2 = ones(n_spines,1)+.5*randomness*rand(n_spines,1);
us2 = ones(n_spines,1)+.5*randomness*rand(n_spines,1);%zeros(n_spines,1);
ws2 = ones(n_spines,1)+.5*randomness*rand(n_spines,1);%zeros(n_spines,1);
sigma2(c2) = sigma1(c1);
sigma2 = sigma2(c2)*ones(n_spines,1)+.5*randomness*rand(n_spines,1);
save('sigma2_0.mat','sigma2')
rhos2 = ones(n_spines,1)-sigma2/Cs+.5*randomness*rand(n_spines,1);
rhos2_minus1 = rhos2+.5*randomness*rand(n_spines,1);
...
if theta_m > change
    for count = 1:length_of_var
        ...
        aux_store = spine_lengths(count);
        spine_lengths(count) = spine_lengths(count)*(1+pert);
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%
        R = Res_finder_BDF2_A_v39(n_v_near,n_p_near,n_spines_near, ...
    ...
    for count = 1:length_of_var
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%
        R = Res_finder_BDF2_noA_v39(n_v_near,n_p_near,n_spines_near, ...
                         n_v_far,n_p_far,n_spines_far, ...
                         theta_c, ...
                         u_bar,w_bar,p, ...
                         lambda2,us2,ws2,rhos2,sigma2, ...
...
for count = 1:length_of_var
    ...
    diff_num(:,count) = (R_forw(:,count)-R_back(:,count)) ...
                        /(2*spine_lengths(count)*pert);
end
...
if theta_m > change
    JR = Jacobian_finder_BDF2_A_v39(n_v_near,n_p_near, ...
                       n_spines_near, ...
                       n_spines_far, ...
    ...
    JR = Jacobian_finder_BDF2_noA_v39(n_v_near,n_p_near, ...
                           n_spines_near, ...
                           n_spines_far, ...
                           n_lambda2_uk_near,n_sigma2_uk_near, ...
                           n_v_sep,n_v_far,n_p_far, ...
...
title('Relative error in $\partial_{h_{near}}$', ...
      'interpreter','latex','FontSize',16)
subplot(1,2,2)
surf((rel_err>1E-4) ...
     .*abs(aux1-aux2),...
     'LineStyle','none','FaceColor','interp')
title(['Absolute error of $\partial_{h_{near}}$ ', ...
