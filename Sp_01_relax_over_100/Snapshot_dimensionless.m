...
for step = floor([it/20 it/2 3*it/4 it-2])
    ...
    for ii = 1:n3_el
        ...
        plot(xvec,yvec,'r','LineWidth',4)
    end
    set(gca,'FontSize',24)
    axis equal
    set(gca,'xlim',[0 2],'ylim',[0 2],'FontSize',24,'xtick',0:.2:2)
    text(-.25,2.05,[labels(count),')'],'FontSize',24)
    grid on
    set(gcf, 'Position', get(0, 'Screensize'));
    title(['$\sigma_{1,e}t/(\rho\nu R) =$ ',sprintf('%8.3f',time_vec(step+1))],...
...
%     writeVideo(vidObj,currFrame);
    print('-depsc',['Spreading',num2str(count),'.eps'])
end
% close(vidObj)