...
if theta_m > change
    for count = 1:length_of_var
        ...
        aux_store = spine_lengths(n_spines_near+count);
        spine_lengths(n_spines_near+count) = spine_lengths(n_spines_near+count)*(1+pert);
        ...
        spine_lengths(n_spines_near+count) = aux_store;

        %Backward perturbation
        aux_store = spine_lengths(n_spines_near+count);
        spine_lengths(n_spines_near+count) = ...
            spine_lengths(n_spines_near+count)*(1-pert);
...
for count = 1:length_of_var
    diff_num(:,count) = (R_forw(:,count)-R_back(:,count)) ...
                        /(2*spine_lengths(n_spines_near+count)*pert);
end
...
title('Relative error in $\partial_{h_{far}}$', ...
      'interpreter','latex','FontSize',16)
subplot(1,2,2)
surf((rel_err>1E-4) ...
     .*abs(aux1-aux2),...
     'LineStyle','none','FaceColor','interp')
title(['Absolute error of $\partial_{h_{far}}$ ', ...
