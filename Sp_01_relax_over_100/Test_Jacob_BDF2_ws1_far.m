...
w_bar = ones(n_v_near,1)+.5*rand(n_v_near,1); 
% -1*(1-exp(-Nodes_rz(1:n_v_near,2)));%
u_far = ones(n_v-n_v_pre_start_far,1)+.5*rand(n_v-n_v_pre_start_far,1);
w_far = ones(n_v-n_v_pre_start_far,1)+.5*rand(n_v-n_v_pre_start_far,1);
% -1*(1-exp(-Nodes_rz(n_v_start_far:end,2)));%
...
gamma3 = ones(n_spines_incr,1)+.5*rand(n_spines_incr,1);
%this is assumed identically null
...
if theta_m > change
    for count = 1:length_of_var
        ...
        aux_store = ws1(n_spines_near+count);
        ws1(n_spines_near+count) = ...
            ws1(n_spines_near+count) + pert;
        ...
        ws1(n_spines_near+count) = aux_store;
        
        %Backward perturbation
        aux_store = ws1(n_spines_near+count);
        ws1(n_spines_near+count) = ws1(n_spines_near+count) - pert;
...
title('Relative error in $\partial_{w^{s_1}_{far}}$', ...
      'interpreter','latex','FontSize',16)
subplot(1,2,2)
surf((rel_err>1E-4) ...
     .*abs(aux1-aux2),...
     'LineStyle','none','FaceColor','interp')
title(['Absolute error of $\partial_{w^{s_1}_{far}}$ ', ...
