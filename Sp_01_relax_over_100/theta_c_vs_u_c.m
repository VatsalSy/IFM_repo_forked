...
xlabel('$U_{cl}[m/s]$','interpreter','latex','FontSize',40)
...
yyaxis right
rhos1_of_U = plot(U_cl_vec(1:it+1)*U_dim,rhos1_cl(1:it+1),'-k','LineWidth',4)
load('rhos_0_dim.mat')
load('rhos1_e_dim.mat')
rhos1_eq = plot([1.1*min(U_cl_vec)*U_dim 1.1*max(U_cl_vec)*U_dim], ...
                [rhos1_e_dim/rhos_0_dim rhos1_e_dim/rhos_0_dim],'--k','Linewidth',4)
rhos2_of_U = plot(U_cl_vec(1:it+1)*U_dim,rhos2_cl(1:it+1),'-','color',[0 .5 0],'LineWidth',4)
load('rhos2_e_dim.mat')
rhos2_eq = plot([1.1*min(U_cl_vec)*U_dim 1.1*max(U_cl_vec)*U_dim], ...
                [rhos2_e_dim/rhos_0_dim rhos2_e_dim/rhos_0_dim],'--','color',[0 .5 0],'Linewidth',4)
set(gca,'xlim',[1.1*min(U_cl_vec(1:it+1))*U_dim 1.1*max(U_cl_vec(1:it+1))*U_dim], ...
...
index_cut = find(time_vec*T_dim*1E6>1.5,1);
time_vec_aux = [time_vec(1:index_cut-1),1.5/(T_dim*1E6)];
A_vec_aux = [A_vec(1:index_cut-1),(A_vec(index_cut-1)+A_vec(index_cut))/2];
figure
yyaxis left
plot(time_vec(1:it+1)*T_dim*1E6,theta_c_vec(1:it+1)*180/pi,'linewidth',9)
hold on
grid on
plot(time_vec(1:it+1)*T_dim*1E6,theta_c_mesh_vec(1:it+1)*180/pi,'-','color',[0 .8 0],'linewidth',3)
set(gca,'FontSize',16,'xlim',[0 1.5],'ylim',[60 160])
text(-.35,165,' c)','FontSize',20)
xlabel('$t[\mu$s$]$','interpreter','latex','FontSize',24)
ylabel('$\theta_c[^o]\ \ \ \ $','interpreter','LaTeX','rotation',90,'FontSize',24)
yyaxis right
plot(time_vec_aux*T_dim*1E6,A_vec_aux,'LineWidth',2,'color',[0 0 0])
set(gca,'ylim',[-2000 4000],'xlim',[0 1.5])
...
set(gca,'ylim',[86 98])
yyaxis right
plot(time_vec(1:index_cut-1)*T_dim*1E6,A_vec(1:index_cut-1),'LineWidth',2,'color',[0 0 0])
set(gca,'xlim',[0.08 .12])
print('-depsc','theta_c_f_of_time.eps')
pause(2)
   
figure
yyaxis left
plot(time_vec(1:it+1)*T_dim*1E9,theta_c_vec(1:it+1)*180/pi,'linewidth',9)
hold on
grid on
plot(time_vec(1:it+1)*T_dim*1E9,theta_c_mesh_vec(1:it+1)*180/pi,'-','color',[0 .8 0],'linewidth',3)
set(gca,'FontSize',16,'xlim',[0 20],'ylim',[60 160])
text(-4.5,165,' d)','FontSize',20)
xlabel('$t[n$s$]$','interpreter','latex','FontSize',24)
ylabel('$\ $','interpreter','LaTeX','rotation',90,'FontSize',24)
yyaxis right
plot(time_vec(1:it+1)*T_dim*1E9,A_vec,'LineWidth',2,'color',[0 0 0])
% set(gca,'ylim',[-2000 4000])
ylabel('$A$','interpreter','LaTeX','rotation',90,'FontSize',40)
legend('$\theta_c$','$\theta_{c,m}$','$A$', ...
...
figure
yyaxis left
plot(time_vec(1:it+1)*T_dim*1E6,U_cl_vec(1:it+1)*U_dim,'linewidth',4)
hold on
grid on
set(gca,'FontSize',16,'xlim',[0 1.5])
text(-.2,8.5,'a)','FontSize',20)
% xlabel('$t[n$s$]$','interpreter','latex','FontSize',24)
ylabel('$U_{cl}[m/s]$','interpreter','LaTeX','rotation',90,'FontSize',24)
yyaxis right
plot(time_vec(1:it+1)*T_dim*1E6,apex_w(1:it+1)*U_dim*1E6,'linewidth',2,'color',[0 .5 0])
...
hold on
grid on
text(-40,8.5,'b)','FontSize',20)
set(gca,'FontSize',16,'xlim',[0 200])
% xlabel('$t[n$s$]$','interpreter','latex','FontSize',24)
ylabel('$\ $','interpreter','LaTeX','rotation',90,'FontSize',40)
yyaxis right
plot(time_vec(1:it+1)*T_dim*1E9,apex_w(1:it+1)*U_dim*1E6,'linewidth',2,'color',[0 .8 0])
ylabel('$w^s_{1,a}\,[\mu m/s]$','interpreter','LaTeX','rotation',90,'FontSize',24)
