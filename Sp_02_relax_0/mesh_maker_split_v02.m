...
n_el_incr = ((1+n_spines_incr-2)/2)*(n_spines_incr-1)/2; %Number of 
...
n_el = n_el_incr + (n_spines-n_spines_incr)*(n_spines_incr-1)/2;%the total 
%number of elements. This is a geometric progression sum based on the fact 
%that every odd-numbered spine (except for the last) has a number of 
%elements in front of it (to their left, assuming contact line is to the 
%right) that is equal to its spine number
save('n_el.mat','n_el')%is equal to the sum of nodes in every odd-numbered 
%spine but the last one

n_spines_near = 35;%This has to be an odd number
...
if n_spines_near > n_spines_incr
    n_v_near =  (1+n_spines_incr)*(n_spines_incr)/2 ...
              + (n_spines_near-n_spines_incr)*n_spines_incr;
    n_v_start_far = n_v_near - n_spines_incr+1;
    n_v_far = n_v - n_v_near + n_spines_incr;
    %Calculating the number of elements in the near field region
    n_el_near =  ((1+n_spines_near-2)/2)*(n_spines_near-1)/2 ...
               + (n_spines_near-n_spines_incr)*n_spines_incr/2;
else
    n_v_near = (1+n_spines_near)*(n_spines_near)/2;
    n_v_start_far = n_v_near - n_spines_near+1;%the first of the nodes in the region
    %that lies far from the contac line
    n_v_far = n_v - n_v_near + n_spines_near;
    %Calculating the number of elements in the near field region
    n_el_near = ((1+n_spines_near-2)/2)*(n_spines_near-1)/2;
end
...
ee = 0;%element counter, number of the last added element
next_foot_node = 1; %this is just to initialise the current foot node 
%counter defined below
for ss = 1:2:n_spines_incr-2 % go through every odd number spine but the 
    %last
    current_foot_node = next_foot_node; %this is the node number of the 
    %corresponding spine foot
    middle_foot_node = current_foot_node+ss; %node number of the foot of 
    %the next spine (we call it middle because it is in between of two 
    %spines that our counter actually goes to)
    next_foot_node = current_foot_node+2*ss+1; %node number of the foot of 
    %the one after next spine
    for jj = 1:2:ss-2 %within each chosen spine, we go through every one of
        %its corner nodes (i.e. taking every other node) but the last each
        %one of this nodes is associate to two elements to its upper left 
        %except for the last which is only associated to one element
        %
...
for ss = n_spines_incr:2:n_spines-2 % go through every odd number spine but 
    ...
    current_foot_node = next_foot_node; %this is the node number of the 
    %corresponding spine foot
    middle_foot_node = current_foot_node+n_spines_incr; %node number of the 
    %foot of the next spine (we call it middle because it is in between of 
    %two spines that our counter actually goes to)
    next_foot_node = current_foot_node+2*n_spines_incr; %node number of the 
    %foot of the one after next spine
    for jj = 1:2:n_spines_incr-2 %within each chosen spine, we go through 
        %every one of its corner nodes (i.e. taking every other node) but 
        %the last each one of this nodes is associate to two elements to 
...
n_u_uk_mixed = n_spines_near;
save('n_u_uk_mixed.mat','n_u_uk_mixed')
unodes_nums_uk_mixed = n_v_near-n_spines_near+1:n_v_near;
save('unodes_nums_uk_mixed.mat','unodes_nums_uk_mixed')

n_u_k_far = n_spines_incr; 
save('n_u_k_far.mat','n_u_k_far')
unodes_nums_k_far  = n_v-n_spines_incr+1:n_v; 
save('unodes_nums_k_far.mat','unodes_nums_k_far')

n_u_uk_far  = n_v_far-n_spines_incr; 
save('n_u_uk_far.mat','n_u_uk_far')
unodes_nums_uk_far  = n_v_start_far:n_v-n_spines_incr; 
save('unodes_nums_uk_far.mat','unodes_nums_uk_far')

n_u_uk_unmixed_far = n_v_far-n_spines_near-n_spines_incr;
save('n_u_uk_unmixed_far.mat','n_u_uk_unmixed_far')
unodes_nums_uk_unmixed_far = n_v_start_far+n_spines_near:n_v-n_spines_incr;
save('unodes_nums_uk_unmixed_far.mat','unodes_nums_uk_unmixed_far')
...
% wnodes_nums_uk_near = zeros(1,n_w_uk_near); 
wnodes_nums_k_near = [];%zeros(1,n_w_k_near);
% wnodes_nums_k_near(1:2) = 1:2;
% wnodes_nums_uk_near(1) = 3;
kk = 3;%number of the last node that has been classified
jj = 1;%index of the last loaded element in unknown-w nodes vector
ll = 2;%index of the last loaded element in known-w nodes vector
for ss = 3:n_spines_near-1
%     wnodes_nums_k_near(ll+1) = kk+1;
    ll = ll+1;
%     wnodes_nums_uk_near(jj+1:jj+ss-1) = kk+2:kk+ss;
    jj = jj+ss-1;
    kk = kk+ss;
end

n_w_k_mixed = 1;
save('n_w_k_mixed.mat','n_w_k_mixed')
wnodes_nums_k_mixed = kk+1;
save('wnodes_nums_k_mixed.mat','wnodes_nums_k_mixed')

ss = n_spines_near;
n_w_uk_mixed = n_spines_near-1;
save('n_w_uk_mixed.mat','n_w_uk_mixed')
wnodes_nums_uk_mixed = kk+2:kk+ss;
save('wnodes_nums_uk_mixed.mat','wnodes_nums_uk_mixed')

wnodes_nums_uk_far = n_v_start_far:n_v;
wnodes_nums_k_far = [];

% wnodes_nums_uk_far = zeros(1,n_w_uk_far); 
% wnodes_nums_k_far = zeros(1,n_w_k_far);
% jj = 0; 
% ll = 0;
% for ss = n_spines_near:n_spines_incr
%     wnodes_nums_k_far(ll+1) = kk+1;
%     ll = ll+1;
%     wnodes_nums_uk_far(jj+1:jj+ss-1) = kk+2:kk+ss;
%     jj = jj+ss-1;
%     kk = kk+ss;
% end
% for ss = n_spines_incr+1:n_spines-1
%     wnodes_nums_k_far(ll+1) = kk+1;
%     ll = ll+1;
%     wnodes_nums_uk_far(jj+1:jj+n_spines_incr-1) = kk+2:kk+n_spines_incr;
%     jj = jj+n_spines_incr-1;
%     kk = kk+n_spines_incr;
% end
% wnodes_nums_k_far(ll+1) = kk+1;
% wnodes_nums_uk_far(jj+1:jj+n_spines_incr-1) = kk+2:kk+n_spines_incr;
...
n_w_uk_mixed = n_spines_near-1;
save('n_w_uk_mixed.mat','n_w_uk_mixed')
wnodes_nums_uk_mixed = n_v_start_far+1:n_v_start_far+n_spines_near-1;
save('wnodes_nums_uk_mixed.mat','wnodes_nums_uk_mixed')


n_w_uk_unmixed_far = n_v_far-n_spines_far;
save('n_w_uk_unmixed_far.mat','n_w_uk_unmixed_far')
wnodes_nums_uk_unmixed_far = wnodes_nums_uk_far(n_spines_near:end);
save('wnodes_nums_uk_unmixed_far.mat','wnodes_nums_uk_unmixed_far')

Mr_nodes_mixed = unodes_nums_uk_mixed;
save('Mr_nodes_mixed.mat','Mr_nodes_mixed')

Mr_nodes_unmixed_far = unodes_nums_uk_unmixed_far;
save('Mr_nodes_unmixed_far.mat','Mr_nodes_unmixed_far')

Mz_nodes_mixed = wnodes_nums_uk_mixed;
save('Mz_nodes_mixed.mat','Mz_nodes_mixed')

Mz_nodes_unmixed_far = wnodes_nums_uk_unmixed_far;
save('Mz_nodes_unmixed_far.mat','Mz_nodes_unmixed_far')
...
figure
plot(Nodes_rz(n_v_near-n_spines_near+1:end,1),Nodes_rz(n_v_near-n_spines_near+1:end,2),'xk')
hold on
...
figure
plot(Nodes_rz(n_v_near-n_spines_near+1:n_v,1),Nodes_rz(n_v_near-n_spines_near+1:n_v,2),'xk')
hold on
...
%The next bit I commented because I think we will not need
% lambda2_b2nodes = 1:n_spines-1;
% save('lambda2_b2nodes.mat','lambda2_b2nodes')
% n_lambda2 = n_spines-1;
% save('n_lambda2.mat','n_lambda2')
% 
% lambda3_b3nodes = n_spines;
% save('lambda3_b3nodes.mat','lambda3_b3nodes')
% n_lambda3 = 1;
% save('n_lambda3.mat','n_lambda3')
% 
% gamma3_b3nodes = n_spines;
% save('gamma3_b3nodes.mat','gamma3_b3nodes')
% n_gamma3 = 1;
% save('n_gamma3.mat','n_gamma3')
% 
% h_uk = 1:n_spines-1;
% save('h_uk.mat','h_uk')
% n_h_uk = n_spines-1;
% save('n_h_uk.mat','n_h_uk')
...
%numbers of the i-th element, the jj-th column corresponds to the node
%number of the jj-th local element node
kk = 0;%element counter, number of the last added element
next_foot_node = 1;
for ss = 1:2:n_spines_incr-2 % go through every odd number spine but the last
    current_foot_node = next_foot_node;%this is the pressure-node number of
    %the corresponding spine foot
    next_foot_node = current_foot_node+(ss+1)/2; %pressure-node number of 
    %the foot of the one after next spine
    for jj = 1:(ss-1)/2 %within each chosen spine, we go through every one 
...
for ss = n_spines_incr:2:n_spines-2 % go n_nodes_incrthrough every odd 
    ...
    current_foot_node = next_foot_node;%this is the pressure-node number of
    %the corresponding spine foot
    next_foot_node = current_foot_node+(n_spines_incr+1)/2; %pressure-node 
    %number of the foot of the one after next spine
    for jj = 1:(n_spines_incr-1)/2 %within each chosen spine, we go through
...
n_p = (n_spines_incr+3)*(n_spines_incr+1)/8 ...
      + (n_spines-n_spines_incr)*(n_spines_incr+1)/4;
save('n_p.mat','n_p')
n_p_near = (n_spines_near+3)*(n_spines_near+1)/8;
save('n_p_near.mat','n_p_near')
n_p_start_far  = n_p_near - (n_spines_near+1)/2 + 1;
save('n_p_start_far.mat','n_p_start_far')
n_p_far = n_p - n_p_near + (n_spines_near+1)/2;
...
n_p_uk_mixed = (n_spines_near+1)/2;
save('n_p_uk_mixed.mat','n_p_uk_mixed')
pnodes_nums_uk_mixed =  n_p_near-(n_spines_near+1)/2+1 : n_p_near;
save('pnodes_nums_uk_mixed.mat','pnodes_nums_uk_mixed')

n_p_uk_far = n_p-n_p_near+(n_spines_near+1)/2;
save('n_p_uk_far.mat','n_p_uk_far')
%number of nodes where the pressure is given

n_p_k_far = 0;
save('n_p_k_far.mat','n_p_k_far')

pnodes_nums_uk_far = n_p_near-(n_spines_near+1)/2+1:n_p;
...
n_p_uk_unmixed_far = n_p_far-(n_spines_near+1)/2;
save('n_p_uk_unmixed_far.mat','n_p_uk_unmixed_far')
pnodes_nums_uk_unmixed_far = n_p_start_far+(n_spines_near+1)/2:n_p;
save('pnodes_nums_uk_unmixed_far.mat','pnodes_nums_uk_unmixed_far')

C_nodes_mixed = pnodes_nums_uk_mixed;
save('C_nodes_mixed.mat','C_nodes_mixed')
C_nodes_unmixed_far = pnodes_nums_uk_unmixed_far;
save('C_nodes_unmixed_far.mat','C_nodes_unmixed_far')
...
kk = 0;
last = 1;
for ss = 1:2:n_spines_incr-2
    kk = kk+1;
    l_1(kk,1) = last;
    l_1(kk,2) = last+ss+1;
    l_1(kk,3) = last+2*ss+3;
    last = last+2*ss+3;
end
for ss = n_spines_incr:2:n_spines-2
    kk = kk+1;
    l_1(kk,1) = last;
    l_1(kk,2) = last+n_spines_incr;
    l_1(kk,3) = last+2*n_spines_incr;
    last = last + 2*n_spines_incr;
end
save('l_1.mat','l_1')
...
for ss = n_spines_incr:2:n_spines-2
    ...
    %numbering from right to left
    l_2(kk,1) = last;
    l_2(kk,2) = last+n_spines_incr;
    l_2(kk,3) = last+2*n_spines_incr;
    last = last+2*n_spines_incr;
end
save('l_2.mat','l_2')
l2_2 = zeros(n2_el,3);
...
save('l3_3.mat','l3_3')
S_3 = n_spines*ones((n_spines_incr-1)/2,1);
save('S_3.mat','S_3')
el3_s_loc_nodes = cell((n_spines_incr-1)/2,1);
for ee = 1:(n_spines_incr-1)/2
    el3_s_loc_nodes{ee,1} = [1 2 3];
end
save('el3_s_loc_nodes.mat','el3_s_loc_nodes')


n_v_sep = n_spines_near;
...
W_G = 2*W_G; %the weights are for a uni area triangle, so they need to be 
...
n_Mr_mixed = n_spines_near;
save('n_Mr_mixed.mat','n_Mr_mixed')
n_Mz_mixed = n_spines_near-1;
save('n_Mz_mixed.mat','n_Mz_mixed')

n_Mr_unmixed_far = n_Mr_far-n_Mr_mixed;
save('n_Mr_unmixed_far.mat','n_Mr_unmixed_far')
n_Mz_unmixed_far = n_Mz_far-n_Mz_mixed;
save('n_Mz_unmixed_far.mat','n_Mz_unmixed_far')

n_C_mixed = (n_spines_near+1)/2;
save('n_C_mixed.mat','n_C_mixed')
n_C_unmixed_far = n_p_far - n_C_mixed;
save('n_C_unmixed_far.mat','n_C_unmixed_far')
