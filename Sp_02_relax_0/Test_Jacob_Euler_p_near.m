...
lambda3 = ones(n_nodes_const_per_spine,1)+.5*rand(n_nodes_const_per_spine,1);
gamma3 = ones(n_nodes_const_per_spine,1)+.5*rand(n_nodes_const_per_spine,1);%this is assumed identically null
...
if theta_m > change  
    for count = 1:length_of_var
        ...
        aux_store = p(count);
        p(count) = p(count) + pert;

        R = Res_finder_Euler_A_v39(n_v_near,n_p_near,n_spines_near, ...
    ...
    for count = 1:length_of_var
        ...
        R = Res_finder_Euler_noA_v39(n_v_near,n_p_near, ...
                      n_spines_near, ...
                      n_v_far,n_p_far,n_spines_far, ...
                      theta_c, ...
                      u_bar,w_bar,p, ...
...
if theta_m > change
    JR = Jacobian_finder_Euler_A_v39(n_v_near,n_p_near,n_spines_near, ...
            n_spines_far, ...
    ...
    JR = Jacobian_finder_Euler_noA_v39(n_v_near,n_p_near, ...
                                n_spines_near, ...
                                n_spines_far, ...
                                n_lambda2_uk_near,n_sigma2_uk_near, ...
                                n_v_sep,n_v_far,n_p_far, ...
...
%%
%p_near
aux1 = JR(:,n_u_uk_near+n_w_uk_near+1:n_u_uk_near+n_w_uk_near+n_p_uk_near);
aux2 = diff_num;
...
title('Relative error in $\partial_{p_{near}}$', ...
      'interpreter','latex','FontSize',16)
subplot(1,2,2)
surf((rel_err>1E-4) ...
     .*abs(aux1-aux2),...
     'LineStyle','none','FaceColor','interp')
title(['Absolute error of $\partial_{p_{near}}$ ', ...
